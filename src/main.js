...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelector("#action-spawn-raw")
    .addEventListener("click", () => spawn_raw())

  document
    .querySelector("#action-execute")
    .addEventListener("click", () => execute())
  document
    .querySelector("#action-execute-raw")
    .addEventListener("click", () => execute_raw())
});


async function spawn_raw() {
...
async function spawn() {
  const command = new TAURI.shell.Command('ipconfig', []);
  command.on('close', data => {
    console.log(`command finished with code ${data.code} and signal ${data.signal}`)
  });
  command.on('error', error => console.error(`command error: "${error}"`));
